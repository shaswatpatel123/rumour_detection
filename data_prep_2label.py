...
def getGData(tweet_data, augmented_data):
    ...
    for i, j in enumerate(augmented_data['featureMatrix']):
        text_feat = feature_extractor.get_bert_features(normalizeTweet(j[0]) )
        social_feat = j[1:]  # Social features
        features.append( [*text_feat, *social_feat] )
...
for event in os.listdir(os.path.join(SAVE_DIR, "pheme")):
    ...
    if len(rumourLabel) < maximum and len(rumourLabel) != 0:
        ...
        graphList.extend(dataAugmentation(rumourLabel, num - 1, rem, AUG_PERC))
    ...
    if len(nonrumourLabel) < maximum and len(nonrumourLabel) != 0:
        ...
        graphList.extend(dataAugmentation(
            nonrumourLabel, num - 1, rem, AUG_PERC))
