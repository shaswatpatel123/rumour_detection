...
def nlpAugmentation(json_file, num, feat_extractor, p_commets_aug=0.15):
    res = []
...
def nlpAugmentationImproved(json_file, num, feat_extractor, p_commets_aug=0.15):
    res = []
    with open(json_file, encoding="utf8") as ofile:
        data = json.load(ofile)
...
def dataAugmentation(data_list, num, rem, feat_extractor, p_commets_aug=0.15, improved=False):
    graphList = []
    if num >= 1:
        print("NUM")
...
def getAugmentedData3Label(SAVE_DIR, TWEET_FEAT, improved, feat_extractor, AUG_PERC=0.15):
    if improved:
...
def getAugmentedData2Label(SAVE_DIR, TWEET_FEAT, improved, feat_extractor, AUG_PERC=0.15):
    AUG_PERC = 0.15
